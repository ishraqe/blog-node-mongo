...
app.get("/blogs/category/:id", (req, res) => {
  ...
  Blog.getBlogByCategoryId(category_id)
    .then(blog => {
      if (!blog) {
        res.status(404).send("blog not found");
      } else {
...
app.patch("/users/blog/:id/edit", authenticate, (req, res) => {
  ...
  Blog.isOwner(id, user_id._id)
    .then(is_owner => {
      if (!is_owner) {
        res.send("doesn't has any auth");
      } else {
        Blog.findByIdAndUpdate(id, { $set: info }, { new: true })
          .then(blog => {
            if (!blog) {
              return res.status(404).send();
            }
...
app.patch("/users/categories/:id", authenticate, (req, res) => {
  ...
  //check if the user is owener or not
  Category.findByIdAndUpdate(id, { $set: info }, { new: true })
    .then(category => {
      if (!category) {
        return res.status(404).send();
      }
...
app.listen(port, () => {
  console.log("started on this port 8000");
});
